...
for (let i = 0; i < menu.length; i++) {
	if (categories.includes(menu[i].category) == false) {
...
categories.forEach((categories) => {
	buttons.push(`<button type="button" class="btn-inline">${categories}</button>`);
});
console.log(buttons);
btnContainer.innerHTML = buttons.join("");
let generatedButtons = document.querySelectorAll(".btn-inline");
generatedButtons.forEach((button) => {
	button.addEventListener("click", function (event) {
		const currentCategory = event.currentTarget.textContent;
		let menuCategory = [];
